...
// Probability of acc (s +, d/t +, v-): 0.33*(1 + w_1 * s + w_2 * d/t - w_3 * v)
// Probability of brk (s -, d/t -, v+): 0.33*(1 - w_1 * s - w_2 * d/t + w_3 * v)
...
//	p_acc = (1 + Math.max(Math.min(w[0]*s_ + w[1]*d_ + w[2]*t_ + w[3]*v_, 1),-1))/3;
//	p_brk = (1 - Math.max(Math.min(w[0]*s_ + w[1]*d_ + w[2]*t_ + w[3]*v_, 1),-1))/3;
...
//	var non_elite_pop_id = f.sortIndices.slice(n_elite);
	...
	for(var i=0;i<non_elite_pop.length;i++){
		...
		parent.push(pop[iter][id].slice(0));
...
function mutation(){
	var mutation_rate = 1.0/Math.sqrt(Math.sqrt(iter+1));
	for(var i=0;i<n_mutate;i++){
		if(Math.random()<mutation_rate){
			mutate_pop[i][0] += Math.round((Math.random()-0.5)*30);
			mutate_pop[i][0] = Math.min(Math.max(mutate_pop[i][0], 10), 40);
		}
		for(var j=1;j<n_var;j++){
			if(Math.random()<mutation_rate){
				mutate_pop[i][j] += (Math.random()-0.5)*0.2;
			}
		}
	}
};
function calculate_fitness(callback){
	...
	var f2 = function(i){
		...
		if(i<pop_size){
			...
			sortWithIndeces(fitness_sorted);
			bestfitness.push(fitness_sorted[0]);
			bestfr.push(pop[iter][fitness_sorted.sortIndices[0]][0]);
			
        	$.post('/adddata_learning',{
				   'score':fitness_sorted[0],
				   'keys':JSON.stringify(pop[iter][fitness_sorted.sortIndices[0]].slice(1)),
				   'finaldrive':fr,
				   'iteration':iter,
				   'method':'ga_noinfo'});
	...
	if (iter==0){
		...
		fitness[iter].push(fitness_sorted[0]); // one elite
		run_game(pop[iter][1], 1, f2);
	}
};
function converge(){
...
var c = 0;
var SCORE = 0;
function run_game(input, i, callback){
...
//				   'method':'user_model_1_1',
				   'method':'inverse_data_no_user_1', // ego: normal ego algoirthm;  player_parameter: to rerun all players using the parametric control model
				   ...
				   'database':'ecoracer_learning_ga_table'},
				   function(){
			        	if (typeof(callback) == 'function') {
			                callback();
			            }
