...
/******************************************** get EGO performance for original game ******************************************/
var EGOperformance = new Array(202);
var maxEGOperformance = new Array(202);
var mean_maxEGOperformance = new Array(202);
//var countEGOperformance = new Array(200);
var getEGOperformance = function(){
	$.post('/readdata_learning',{'database':'ecoracer_learning_ego_table'},function(res){
		data = res.data;
		$.each(data,function(i,d){
			if(typeof EGOperformance[d.iteration] == 'undefined'){
				EGOperformance[d.iteration] = [];
//				countEGOperformance[d.iteration] = [];
			}
			EGOperformance[d.iteration].push(Math.max(d.score,0));
//			countEGOperformance[d.iteration]+=1;
		});
		$.each(EGOperformance,function(i,d){
			if(i>0){
				 temp = EGOperformance[i].concat(maxEGOperformance[i-1]);
				 temp.sort(function(a, b){return b-a}); //sort from large to small
				 maxEGOperformance[i] = temp.slice(0,EGOperformance[i].length);
			}
			else{
				temp = EGOperformance[i];
				temp.sort(function(a, b){return b-a}); //sort from large to small
				maxEGOperformance[i] = temp.slice(0,10);
			}
			temp = 0;
			$.each(maxEGOperformance[i], function(ii,dd){
				temp += dd;
			});
			mean_maxEGOperformance[i] = temp/maxEGOperformance[i].length;
		})
	});
}


/******************************************** get EGO performance for simulated games ******************************************/
...
var EGOperformance = new Array(202);
var getEGOperformance_simulated = function(){
	$.post('/readdata_learning',{'database':'ecoracer_learning_ego_table','method':'zigzag_data_no_user_3'},function(res){
		data = res.data;
		$.each(data,function(i,d){
			EGOperformance[d.iteration] = d.score;
		});
	});
}